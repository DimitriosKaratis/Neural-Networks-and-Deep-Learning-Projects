...
MLP = kerasMLP(num_of_classes, [512,256,128], ['relu', 'relu', 'relu'], dropout_rate=0.3, input_shape=(input_shape,), l2_lambda=0.0)

history = MLP.train(img_train, label_train, img_val, label_val, batch_size=32, epochs=70, patience=10)
...
# Print results
print(f"\nMLP's Accuracy using keras: {accuracy}")
...
MLP.plot_accuracy(history, "mlp.png")
Visualization.generate_conf_matrix(label_test, labels_pred_classes, title="Confusion Matrix for MLP using keras")